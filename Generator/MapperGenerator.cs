...
namespace DynamicsMapper
{
    ...
    public class MapperGenerator : IIncrementalGenerator
    {
        ...
        private static MapperClassDetails GenerateMapperClass(ClassDeclarationSyntax mapperSyntax, string entityName, FieldGenerationDetails[] generationDetails, string mapperClassName, SourceProductionContext ctx)
        {
            ...
            {
                using (writer.BeginScope($"public class {mapperClassName} : IEntityMapper<{@namespace}.{className}> "))
                {
                    ...
                    writer.AppendLine();
                    using (writer.BeginScope($"public Entity Map({@namespace}.{className} {modelName})"))
                    {
                    ...
                    writer.AppendLine();
                    using (writer.BeginScope($"public {@namespace}.{className} Map(Entity entity)"))
                    {
                        ...
                        writer.AppendLine($"var {modelName} = new {@namespace}.{className}();");
                        toModelContent.ForEach(writer.AppendLine);
                        writer.AppendLine($"return {modelName};");
                    }
                    using (writer.BeginScope($"public {@namespace}.{className} FromAliasedEntity(Entity entity, string alias)"))
                    {
